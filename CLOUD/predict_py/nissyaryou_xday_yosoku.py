...
# データの読み込み
water_temp_df = pd.read_csv("scr_csv/nissyaryou_takamatsu.csv", parse_dates=['datetime'], dayfirst=True)
s_target = 'takamatsu'
...
# window_size=365
model=load_model('predict_py/model_from_nissayryou_model_save_win365.h5')
...
def predict_temperature(model, specified_data,weather_df):
    # specified_date = datetime.strptime(specified_data, '%Y/%m/%d')
    print('format')
    ...
    if specified_date in data.index:
        ...
        for day in range(7):
            ...
            if day == 0:
                ...
                print(f"Predicted temperature for {current_date}: {predicted_value_specified_date[-1, 0]+weather_df.loc[day, 'nissyaryou'] }")
                print(weather_df.loc[day, 'nissyaryou'])
                ...
                next_date_srt=datetime.strftime(next_date, '%Y/%#m/%#d')
                print(f"Predicted temperature for {next_date_srt}: {predicted_value_specified_date[-1, 0]+weather_df.loc[day, 'nissyaryou']}")
                print(weather_df.loc[day, 'nissyaryou'])
